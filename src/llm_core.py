...
def _local_model_gate_enabled() -> bool:
    return os.getenv("TELEMACHOS_LOCAL_MODEL_GATE", "true").lower() not in {"0", "false", "no", "off"}
...
def _ollama_normalize_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Telemachos' canonical OpenAI-style messages to native Ollama /api/chat.

    Two shape mismatches silently break requests:

    1. Tool calls: Telemachos carries `function.arguments` as a JSON *string*.
...
def _apply_local_cache_affinity(payload: Dict, url: str, session_id: Optional[str]) -> None:
    ...
    Sending a stable ``session_id`` (derived from the Telemachos session) lets
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        ...
        h.setdefault("X-OpenRouter-Title", "Telemachos")
    if provider == "copilot":
...
# value returns HTTP 400. Telemachos does not currently send the `thinking` mode
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Telemachos-only metadata before sending messages to providers.
