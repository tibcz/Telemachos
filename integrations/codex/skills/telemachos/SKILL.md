---
name: telemachos
description: Use when the user asks Codex to read or write Telemachos data (todos, email, calendar, memory, documents) or to launch/monitor/stop a Cookbook model-serve task through the scoped Codex Agent API. Requires TELEMACHOS_URL and TELEMACHOS_API_TOKEN.
---

# Telemachos

Use this skill when a user asks to interact with Telemachos from Codex.
...
- `TELEMACHOS_API_TOKEN`: Scoped API token created in Telemachos Settings > Integrations > Add Integration > Codex Agent.

If either value is missing, do not guess credentials. Tell the user to create a Codex Agent token in Telemachos Settings and expose both values to the terminal session.
...
- All Telemachos data access MUST go through the scoped HTTP API under `/api/codex/*`.
...
```bash
python3 integrations/codex/scripts/telemachos_api.py capabilities
python3 integrations/codex/scripts/telemachos_api.py todos list
python3 integrations/codex/scripts/telemachos_api.py todos add "Follow up"
```
...
```bash
python3 integrations/codex/scripts/telemachos_api.py emails list 5
python3 integrations/codex/scripts/telemachos_api.py emails read UID
```
...
python3 integrations/codex/scripts/telemachos_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
python3 ~/plugins/telemachos/scripts/telemachos_api.py cookbook output serve-abc12345 400
python3 ~/plugins/telemachos/scripts/telemachos_api.py cookbook stop serve-abc12345
python3 ~/plugins/telemachos/scripts/telemachos_api.py cookbook serve \
...
If you are about to reach the Telemachos host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass Telemachos Settings and token scopes. Ask the user to enable the relevant Codex Agent tool toggle instead.
