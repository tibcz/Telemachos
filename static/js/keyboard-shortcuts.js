...
export function initKeyboardShortcuts(modules) {
  ...
  window._telemachosKeybinds = { ..._defaultKeybinds };

  // Load saved keybinds
  getSettings()
    .then(s => { if (s.keybinds) window._telemachosKeybinds = { ..._defaultKeybinds, ...s.keybinds }; })
    .catch(() => {});
  ...
  document.addEventListener('keydown', (e) => {
    const kb = window._telemachosKeybinds;
      ...
      uiModule.styledConfirm('Delete this session?', { confirmText: 'Delete', danger: true }).then(ok => {
        ...
        fetch(`${API_BASE}/api/session/${sid}`, { method: 'DELETE' }).then(async () => {
          ...
          if (nextSession) {
            ...
            el('current-meta').textContent = 'Telemachos Chat';
