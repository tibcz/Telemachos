...
async function _fetchDependencies() {
  ...
  try {
    ...
    const _appDepsHtml = (items) => {
      ...
      const parts = [_sectionHeader('Telemachos app', 'Run inside the Telemachos app itself.')];
      const order = ['System', 'Tools', 'Image', 'LLM', 'Audio', 'Other'];
      for (const cat of order) {
        const catItems = _sortDeps(byCat.get(cat) || [], cat);
        if (!catItems.length) continue;
  ...
  if (dlBtn && dlInput) {
    ...
    const triggerDownload = async () => {
      ...
      if (looksGgufRepo && !pickerInclude) {
        ...
        if (!pickerInclude) {
          uiModule.showToast('Pick a GGUF quant first. Telemachos will not download the whole GGUF repo without an include pattern.');
          return;
        }
...
function _renderRecipes() {
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend Telemachos capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install Telemachos keys, choose model directories, and set the default server. Local is this machine.</p>';
