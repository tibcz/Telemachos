...
let _sortMode = Storage.get('telemachos-session-sort') || 'active'; // default to last active
...
function _shouldPreserveStartupComposer(msgInput) {
  ...
  if (window.__telemachosComposerUserEdited) return true;
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Telemachos Chat';
...
const FOLDER_STATE_KEY = 'telemachos-folder-state';
const FOLDER_ORDER_KEY = 'telemachos-folder-order';
...
export async function selectSession(id, { keepSidebar = false, showLoading = true, immediateLoading = false } = {}) {
  ...
  try {
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Telemachos Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('telemachos-doc-open-' + id) === '1';
...
export function createDirectChat(url, modelId, endpointId, opts = {}) {
  ...
  currentSessionId = null;
  try { window.__telemachosLastSelectedSessionId = ''; } catch (_) {}
  Storage.remove('lastSessionId');
  history.replaceState(null, '', window.location.pathname);
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('telemachos-session-sort', mode);
  else Storage.remove('telemachos-session-sort');
  renderSessionList();
}
