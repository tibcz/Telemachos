                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Telemachos (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Telemachos" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "telemachos_kind" if "telemachos_kind" in cols else "'scheduled' AS telemachos_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                # externally cron-driven `telemachos-mail poll-scheduled`, or
                # an admin running the CLI manually alongside the in-process
                # one despite the TELEMACHOS_INPROCESS_POLLERS=0 guidance) -
                ...
                telemachos_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Telemachos-Origin"] = "telemachos-ui"
                outer["X-Telemachos-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", telemachos_kind or "scheduled")[:64]
                outer["X-Telemachos-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `telemachos-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    ...
    import os
    raw = os.environ.get("TELEMACHOS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (TELEMACHOS_INPROCESS_POLLERS=0); "
            "drive `telemachos-mail poll-scheduled` externally."
        )
        return
    import asyncio
