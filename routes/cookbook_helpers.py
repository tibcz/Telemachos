...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends Telemachos' own interpreter directory to PATH. If Telemachos itself is
...
def _append_pip_install_runner_lines(runner_lines: list[str], cmd: str) -> None:
    ...
    runner_lines.append('  echo "[telemachos] pip does not support --break-system-packages; installing without it."')
...
def _user_shell_path_bootstrap() -> list[str]:
    return [
        'TELEMACHOS_USER_SHELL="${SHELL:-}"',
        'if [ -n "$TELEMACHOS_USER_SHELL" ] && [ -x "$TELEMACHOS_USER_SHELL" ]; then',
        '  TELEMACHOS_USER_PATH="$("$TELEMACHOS_USER_SHELL" -ic \'printf "__TELEMACHOS_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__TELEMACHOS_PATH__//p\' | tail -n 1 || true)"',
        '  if [ -n "$TELEMACHOS_USER_PATH" ]; then export PATH="$TELEMACHOS_USER_PATH:$PATH"; fi',
        'fi',
...
def _cached_model_scan_script(model_dirs: list[str] | None = None, add_hf_cache: str | None = None) -> str:
    ...
    lines = [
        ...
        "    if os.name == 'nt' and not os.environ.get('TELEMACHOS_ALLOW_OLLAMA_CLI_SCAN'): return",
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by Telemachos.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$TELEMACHOS_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $TELEMACHOS_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        ...
        runner_lines.append('  exit "$TELEMACHOS_PREFLIGHT_EXIT"')
    runner_lines.append('fi')
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for Telemachos-managed `pip install --user`
    # installs, but then report the actual CLI path so external runtimes are clear.
    runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
    runner_lines.append('TELEMACHOS_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"')
    runner_lines.append('if [ -z "$TELEMACHOS_VLLM_BIN" ]; then')
    runner_lines.append('  echo "ERROR: vLLM is not installed."')
    runner_lines.append('  TELEMACHOS_PREFLIGHT_EXIT=127')
    runner_lines.append('else')
    runner_lines.append('  echo "[telemachos] vLLM CLI: $TELEMACHOS_VLLM_BIN"')
    runner_lines.append('  TELEMACHOS_VLLM_VERSION="$("$TELEMACHOS_VLLM_BIN" --version 2>&1 | head -n 1 || true)"')
    runner_lines.append('  if [ -n "$TELEMACHOS_VLLM_VERSION" ]; then echo "[telemachos] vLLM version: $TELEMACHOS_VLLM_VERSION"; fi')
    runner_lines.append('fi')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('TELEMACHOS_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $TELEMACHOS_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $TELEMACHOS_CMD_EXIT ==="')
        ...
        runner_lines.append('echo ""; echo "=== Process exited with code $TELEMACHOS_CMD_EXIT ==="')
        runner_lines.append('exit "$TELEMACHOS_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    # every OS-package dep from the launch path. Sets _telemachos_have_prebuilt=1
    ...
    runner_lines.append('    _telemachos_have_prebuilt=""')
    runner_lines.append('    _telemachos_arch="$(uname -m)"')
    runner_lines.append('    _telemachos_prebuilt_url=""')
    runner_lines.append('    if command -v curl >/dev/null 2>&1 && [ "$_telemachos_arch" = "x86_64" ]; then')
    runner_lines.append('      _telemachos_pat=""')
    runner_lines.append('      _telemachos_has_nv_inline() { command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L 2>/dev/null | grep -q "GPU "; }')
    runner_lines.append('      _telemachos_has_vk_inline() { ldconfig -p 2>/dev/null | grep -q "libvulkan\\.so" || command -v vulkaninfo >/dev/null 2>&1 || [ -e /usr/lib/x86_64-linux-gnu/libvulkan.so.1 ]; }')
    runner_lines.append('      _telemachos_has_vkdev_inline() { ls /dev/dri/renderD* >/dev/null 2>&1 || (lspci 2>/dev/null | grep -Ei \'VGA|3D|Display\' | grep -Eiq \'AMD|ATI|Radeon\'); }')
    runner_lines.append('      if _telemachos_has_nv_inline; then')
    runner_lines.append('        _telemachos_pat="ubuntu.*cuda"')
    runner_lines.append('      elif _telemachos_has_vkdev_inline && _telemachos_has_vk_inline; then')
    runner_lines.append('        _telemachos_pat="ubuntu.*vulkan"')
    runner_lines.append('      else')
    runner_lines.append('        _telemachos_pat="ubuntu-x64\\\\.zip"')
    runner_lines.append('      fi')
    runner_lines.append('      _telemachos_prebuilt_url="$(curl -fsSL --max-time 15 https://api.github.com/repos/ggml-org/llama.cpp/releases/latest 2>/dev/null | grep \'"browser_download_url"\' | cut -d\'"\' -f4 | grep -iE "$_telemachos_pat" | grep -iv "arm\\|aarch64" | head -1)"')
    runner_lines.append('    fi')
    ...
    runner_lines.append('    if [ -n "$_telemachos_prebuilt_url" ] && (command -v unzip >/dev/null 2>&1 || command -v bsdtar >/dev/null 2>&1 || command -v python3 >/dev/null 2>&1); then')
    runner_lines.append('      echo "[telemachos] Found prebuilt llama-server: $_telemachos_prebuilt_url"')
    runner_lines.append('      mkdir -p ~/bin "$HOME/.cache/telemachos/llama-cpp-prebuilt" && cd "$HOME/.cache/telemachos/llama-cpp-prebuilt"')
    runner_lines.append('      rm -f llama-cpp.zip')
    runner_lines.append('      if curl -fsSL --max-time 120 "$_telemachos_prebuilt_url" -o llama-cpp.zip && [ -s llama-cpp.zip ]; then')
    ...
    runner_lines.append('        _telemachos_extracted="$(find build -type f -name llama-server 2>/dev/null | head -1)"')
    runner_lines.append('        if [ -n "$_telemachos_extracted" ]; then')
    runner_lines.append('          chmod +x "$_telemachos_extracted"')
    runner_lines.append('          ln -sf "$_telemachos_extracted" ~/bin/llama-server')
    runner_lines.append('          _telemachos_libdir="$(dirname "$_telemachos_extracted")"')
    runner_lines.append('          mkdir -p ~/.config && echo "export LD_LIBRARY_PATH=\\"$_telemachos_libdir:\\${LD_LIBRARY_PATH:-}\\"" > ~/.config/telemachos-llama-cpp-env')
    runner_lines.append('          _telemachos_have_prebuilt=1')
    ...
    runner_lines.append('    _telemachos_has_nvidia_hw() {')
    ...
    runner_lines.append('    }')
    runner_lines.append('    if _telemachos_has_nvidia_hw; then')
    ...
    runner_lines.append('        echo "[telemachos] Auto-installing missing build deps via apt:$_missing"')
    ...
    runner_lines.append('        echo "[telemachos] Auto-installing missing build deps via pacman:$_missing"')
    ...
    runner_lines.append('        echo "[telemachos] Auto-installing missing build deps via dnf:$_missing"')
    ...
    runner_lines.append('    }')
    runner_lines.append('    _telemachos_apt_bootstrap')
    runner_lines.append('    _telemachos_missing_build_deps=""')
    runner_lines.append('    command -v cmake >/dev/null 2>&1 || _telemachos_missing_build_deps="$_telemachos_missing_build_deps cmake"')
    runner_lines.append('    command -v git >/dev/null 2>&1 || _telemachos_missing_build_deps="$_telemachos_missing_build_deps git"')
    runner_lines.append('    command -v g++ >/dev/null 2>&1 || command -v gcc >/dev/null 2>&1 || _telemachos_missing_build_deps="$_telemachos_missing_build_deps build-essential"')
    runner_lines.append('    if [ -n "$_telemachos_missing_build_deps" ]; then')
    runner_lines.append('      echo "ERROR: llama.cpp source build needs missing packages:$_telemachos_missing_build_deps"')
    ...
    runner_lines.append('    _telemachos_has_vulkan() {')
    ...
    runner_lines.append('      return 1')
    runner_lines.append('    }')
    runner_lines.append('    _telemachos_has_vulkan_device() {')
    ...
    runner_lines.append('      _telemachos_has_cudart() {')
    ...
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
    runner_lines.append('  fi  # end _telemachos_have_prebuilt guard')
...
def _llama_cpp_rebuild_cmd(update_source: bool = False) -> str:
    ...
    if update_source:
        update_cmd = (
            ...
            'echo "[telemachos] WARNING: llama.cpp source update failed; clearing cached build anyway."; '
            'elif command -v git >/dev/null 2>&1; then '
            'git clone --depth 1 https://github.com/ggml-org/llama.cpp "$HOME/llama.cpp" || '
            'echo "[telemachos] WARNING: llama.cpp clone failed; clearing cached build anyway."; '
            'fi && '
        )
    return (
        'mkdir -p "$HOME/bin" && '
        ...
        'echo "[telemachos] Cleared the cached llama.cpp build. '
...
def _diagnose_serve_output(text: str) -> dict | None:
    ...
    patterns = [
        ...
        (
            r"mlx-lama-swift|telemachos-mlx-inpaint|mlx-lama-serve|LaMa / MI-GAN MLX inpainting models require",
            "LaMa / MI-GAN MLX inpainting requires an Telemachos-compatible mlx-lama-swift bridge on this Apple Silicon server.",
            [{"label": "build mlx-lama-swift bridge and put telemachos-mlx-inpaint or mlx-lama-serve on PATH", "op": "dependency", "package": "mlx_lama_swift"}],
        ),
        (
            r"mlx-ddcolor-swift|telemachos-mlx-colorize|mlx-ddcolor-serve|DDColor MLX models require",
            "DDColor MLX colorization requires an Telemachos-compatible mlx-ddcolor-swift bridge on this Apple Silicon server.",
            [{"label": "build mlx-ddcolor-swift bridge and put telemachos-mlx-colorize or mlx-ddcolor-serve on PATH", "op": "dependency", "package": "mlx_ddcolor_swift"}],
        ),
        (
