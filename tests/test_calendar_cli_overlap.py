"""Regression: `telemachos-calendar list` must select events that OVERLAP the
...
`telemachos-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "telemachos-calendar"
    loader = importlib.machinery.SourceFileLoader("telemachos_calendar_cli", str(path))
